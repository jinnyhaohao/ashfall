...
public class WorldPresentation
{
    ...
    public void Update(){if(AshfallBeta.Paused)return;int z=WorldAtlas.Index(d.zone);foreach(int n in WorldAtlas.Neighbors(z))if(Vector2.Distance(d.player.transform.position,WorldAtlas.Centers[z]+WorldAtlas.Gate(z,n))<1.5f&&Input.GetKeyDown(KeyCode.E)){if(WorldAtlas.Unlocked(n,d))beta.Travel(n);else d.Tell("Sealed route: "+WorldAtlas.Requirements[n]+" kills"+(n==7?" and defeat Hollow Knight":"")+" required.");return;}if(Input.GetKeyDown(KeyCode.E)){var node=WorldDetails.Nearest(d);if(node)node.Harvest(d);}}
    ...
    public void Mini(GUIStyle text){var node=WorldDetails.Nearest(d);if(node)GUI.Label(new Rect(450,474,550,28),"E  Gather "+ItemCatalog.Name(node.item),text);int z=WorldAtlas.Index(d.zone);Rect rect=new Rect(1072,177,165,165);Box(new Rect(1057,165,193,209),new Color(.055f,.07f,.13f,.94f));GUI.DrawTexture(rect,maps[z]);foreach(int n in WorldAtlas.Neighbors(z))Dot(WorldAtlas.MapPoint(WorldAtlas.Gate(z,n),rect),Color.cyan);Dot(WorldAtlas.MapPoint((Vector2)d.player.transform.position-WorldAtlas.Centers[z],rect),Color.yellow,7);GUI.Label(new Rect(1070,347,175,23),"M  ATLAS / E  ROUTE",text);foreach(int n in WorldAtlas.Neighbors(z))if(Vector2.Distance(d.player.transform.position,WorldAtlas.Centers[z]+WorldAtlas.Gate(z,n))<2)GUI.Label(new Rect(450,505,550,28),"E  Travel to "+WorldAtlas.Names[n],text);}
    public void Draw(GUIStyle heading,GUIStyle text){int current=WorldAtlas.Index(d.zone);if(selected<0)selected=current;Box(new Rect(88,90,1104,535),new Color(.055f,.065f,.12f,.99f));GUI.Label(new Rect(113,105,700,42),"ASHFALL / EXPLORER'S ATLAS",heading);if(GUI.Button(new Rect(911,112,248,30),local?"Show world routes":"Show local terrain"))local=!local;
        ...
        bool unlocked=WorldAtlas.Unlocked(selected,d);GUI.enabled=unlocked;if(GUI.Button(new Rect(840,534,315,38),"Travel to "+WorldAtlas.Names[selected])){beta.Travel(selected);beta.CloseMap();}GUI.enabled=true;
        GUI.Label(new Rect(113,595,1040,25),unlocked?"Yellow: you   Cyan: exits (E nearby)   Red: monsters   Orange: landmark   M / ESC closes":"Requires "+WorldAtlas.Requirements[selected]+" kills"+(selected==7?" + Hollow Knight defeated":""),text);
    }
}
