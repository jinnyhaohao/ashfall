...
public class AshfallBeta : MonoBehaviour
{
    ...
    bool SprintTesting {get{return Array.IndexOf(Environment.GetCommandLineArgs(),"-sprintTest")>=0;}}
    ...
    bool Testing {get{return ManualTesting||CaptureTesting||DetailTesting||WorldTesting||WeaponTesting||RigTesting||ItemTesting||BetaTesting||SprintTesting;}}
    ...
    void Start()
    {
        ...
        d.blacksmith=gameObject.AddComponent<BlacksmithStation>();d.blacksmith.Initialize(d,art[10]);
        ...
        if(DetailTesting)StartCoroutine(TestDetails());else if(WorldTesting)StartCoroutine(TestWorld());else if(WeaponTesting)StartCoroutine(TestWeapons());else if(ItemTesting)StartCoroutine(TestItems());else if(RigTesting)StartCoroutine(TestRig());else if(SprintTesting)StartCoroutine(TestSprint());else if(BetaTesting)StartCoroutine(TestBeta());
    }
    ...
    IEnumerator TestWorld(){
        ...
        for(int z=0;z<8;z++){
            ...
            Check(z==0?d.enemies.Count==0:d.enemies.Count>=4,z==0?"Safe village hub":"Populated "+zones[z]);
    ...
    IEnumerator TestItems(){
        ...
        p.EquipItem(ItemId.EmberSword);Check(p.Damage==18,"Ember Sword damage");d.Spawn(EnemyKind.Goblin,1);var target=d.enemies[0];p.ModifiedDamage(target,18);Check(target.burnUntil>Time.time,"Burn applied");
    ...
    IEnumerator TestSprint(){
        yield return new WaitForSecondsRealtime(1);title=false;pause=true;d.player.enabled=false;d.kills=60;d.zoneTier=10;d.bossState=2;foreach(ItemId i in Enum.GetValues(typeof(ItemId)))d.bag[i]=1000;
        var types=new HashSet<ObjectiveType>();for(int z=1;z<8;z++)types.Add(d.objectives.Type(z));Check(types.Count==4,"Four reusable objective types assigned");
        TravelForTest(1);foreach(var e in d.enemies)if(e)e.hp=0;for(int slot=0;slot<3;slot++){d.player.transform.position=ExpeditionObjectives.Position(1,slot);Check(d.objectives.Interact(),"Anchor interaction "+slot);}Check(d.objectives.completed[1]&&d.objectives.progress[1]==7,"Corrupted anchors complete and persist as bits");
        pause=false;yield return null;yield return new WaitForEndOfFrame();ScreenCapture.CaptureScreenshot(Path.Combine(Application.dataPath,"../sprint-objective-anchors.png"));pause=true;
        TravelForTest(2);d.player.transform.position=ExpeditionObjectives.Position(2,0);Check(d.objectives.Interact()&&d.objectives.started[2],"Survival wave objective starts");int safety=0;while(!d.objectives.completed[2]&&safety++<5){var targets=new List<EnemyActor>(d.enemies.FindAll(e=>e&&e.objectiveTarget));Check(targets.Count>0,"Survival wave has marked enemies");foreach(var e in targets)if(e)e.TakeDamage(99999);}Check(d.objectives.completed[2]&&d.objectives.progress[2]==3,"Three survival waves complete");
        TravelForTest(3);d.player.transform.position=ExpeditionObjectives.Position(3,0);Check(d.objectives.Interact()&&d.objectives.started[3]&&d.objectives.aux[3]==100,"Relic defense starts at full integrity");d.objectives.TickDefense(.25f);Check(d.objectives.aux[3]>=90&&d.objectives.aux[3]<100,"Relic damage uses a fair per-second rate");foreach(var e in d.enemies)if(e&&e.objectiveTarget)e.transform.position=ExpeditionObjectives.Position(3,0)+Vector2.right*5;d.objectives.TickDefense(30.1f);Check(d.objectives.completed[3]&&d.objectives.aux[3]>0,"Relic defense completes after the survival timer");
        TravelForTest(4);d.player.transform.position=ExpeditionObjectives.Position(4,0);Check(d.objectives.Interact()&&d.objectives.started[4],"Elite hunt starts");var hunted=d.enemies.Find(e=>e&&e.objectiveTarget);Check(hunted&&hunted.modifier!=EliteModifier.None,"Elite hunt marks a modified champion");hunted.TakeDamage(99999);Check(d.objectives.completed[4],"Elite hunt completes on target defeat");
        foreach(var e in new List<EnemyActor>(d.enemies))if(e)Destroy(e.gameObject);d.enemies.Clear();var elites=new List<EnemyActor>();foreach(EliteModifier m in Enum.GetValues(typeof(EliteModifier)))if(m!=EliteModifier.None){d.Spawn(EnemyKind.Goblin,1,m);elites.Add(d.enemies[d.enemies.Count-1]);}Check(elites.Count==5&&elites.TrueForAll(e=>e.modifier!=EliteModifier.None&&e.GetComponent<EliteBadge>()),"Five elite variants have visible badges");for(int i=0;i<elites.Count;i++)elites[i].transform.position=centers[4]+new Vector2(-4+i*2,0);d.player.transform.position=centers[4]+Vector2.down*2;d.cam.transform.position=new Vector3(centers[4].x,centers[4].y,-10);pause=false;yield return null;yield return new WaitForEndOfFrame();ScreenCapture.CaptureScreenshot(Path.Combine(Application.dataPath,"../sprint-elites.png"));pause=true;
        var armored=elites.Find(e=>e.modifier==EliteModifier.Armored);float armoredHp=armored.hp;armored.TakeDamage(10);Check(armoredHp-armored.hp>0&&armoredHp-armored.hp<10,"Armored elite reduces incoming damage");var swift=elites.Find(e=>e.modifier==EliteModifier.Swift);Check(swift.speed>2.2f,"Swift elite gains readable speed behavior");
        var vampire=elites.Find(e=>e.modifier==EliteModifier.Vampiric);vampire.hp=vampire.maxHp-20;d.player.hp=d.player.MaxHp;float vampireHp=vampire.hp;vampire.HitPlayer(10);Check(vampire.hp>vampireHp,"Vampiric elite heals from actual damage");var volatileElite=elites.Find(e=>e.modifier==EliteModifier.Volatile);volatileElite.TakeDamage(99999);Check(FindFirstObjectByType<VolatileBurst>()!=null,"Volatile elite leaves telegraphed blast");
        var warden=elites.Find(e=>e&&e.modifier==EliteModifier.Warden);d.Spawn(EnemyKind.Goblin,1,EliteModifier.None);var warded=d.enemies[d.enemies.Count-1];warded.transform.position=warden.transform.position+Vector3.right;float wardedHp=warded.hp;warded.TakeDamage(10);Check(Mathf.Abs(wardedHp-warded.hp-8)<.01f,"Warden aura protects nearby allies");
        foreach(var e in new List<EnemyActor>(d.enemies))if(e)Destroy(e.gameObject);d.enemies.Clear();d.Spawn(EnemyKind.Goblin,1,EliteModifier.Swift);d.enemies[0].objectiveTarget=true;Save();TravelForTest(0);TravelForTest(4);Check(d.enemies.Exists(e=>e&&e.modifier==EliteModifier.Swift&&e.objectiveTarget),"Elite modifier and objective tag persist between visits");
        TravelForTest(0);d.bag[ItemId.IronSword]=1;d.player.EquipItem(ItemId.IronSword);d.bag[ItemId.Gold]=1000;d.bag[ItemId.IronOre]=100;int baseDamage=d.player.Damage;Check(d.blacksmith.Upgrade(ItemId.IronSword)&&d.player.Damage>baseDamage,"Blacksmith rank increases weapon damage");Save();d.blacksmith.levels[(int)ItemId.IronSword]=0;Load();Check(d.blacksmith.Level(ItemId.IronSword)==1&&d.player.Damage>baseDamage,"Weapon upgrade persists through save/load");
        var legacyItems=new int[Enum.GetValues(typeof(ItemId)).Length];legacyItems[(int)ItemId.IronSword]=1;var legacySeals=new int[8];legacySeals[1]=7;var legacyClaimed=new bool[8];File.WriteAllText(SavePath,JsonUtility.ToJson(new SaveData{version=3,items=legacyItems,seals=legacySeals,claimed=legacyClaimed,sword=(int)ItemId.IronSword}));Load();Check(d.objectives.completed[1]&&d.objectives.progress[1]==7&&d.blacksmith.Level(ItemId.IronSword)==0,"Version 3 save migrates objectives and zero forge ranks");
        d.bag[ItemId.IronSword]=1;d.bag[ItemId.Gold]=1000;d.bag[ItemId.IronOre]=100;d.blacksmith.selected=ItemId.IronSword;d.blacksmith.open=true;pause=false;yield return null;yield return new WaitForEndOfFrame();ScreenCapture.CaptureScreenshot(Path.Combine(Application.dataPath,"../sprint-blacksmith.png"));
        Debug.Log("OBJECTIVE / ELITE / FORGE QA: ALL CHECKS PASSED");yield return new WaitForSecondsRealtime(.5f);Application.Quit();
    }
    IEnumerator TestBeta(){
        ...
        d.Spawn(EnemyKind.Slime,1);var victim=d.enemies[0];d.player.ringA=ItemId.RingVampire;d.player.hp=50;victim.hp=10;victim.TakeDamage(999,ItemId.IronSword);Check(Mathf.Abs(d.player.hp-50.3f)<.01f,"Ring lifesteal excludes overkill");
        d.kills=18;d.zoneTier=3;d.objectives.progress[1]=d.objectives.seals[1]=3;d.objectives.claimed[2]=true;Save();d.kills=0;d.objectives.progress[1]=d.objectives.seals[1]=0;d.objectives.claimed[2]=false;Load();Check(d.kills==18&&d.objectives.progress[1]==3&&d.objectives.claimed[2],"Version 5 save and objective persistence");
        ...
        d.player.transform.position=ExpeditionObjectives.Position(1,2);Check(d.objectives.Interact()&&d.objectives.progress[1]==7,"Final expedition anchor shatters");int rewardGold=d.bag[ItemId.Gold];d.player.transform.position=ExpeditionObjectives.Position(1,3);Check(d.objectives.Interact()&&d.objectives.claimed[1]&&d.bag[ItemId.Gold]>rewardGold,"Guarded expedition cache rewards");
    ...
    void Update()
    {
        ...
        if(Input.GetKeyDown(KeyCode.Escape)){
            if(d.craftOpen||d.inventoryOpen||d.helpOpen||map||(d.blacksmith&&d.blacksmith.open)){d.craftOpen=d.inventoryOpen=d.helpOpen=map=false;if(d.blacksmith)d.blacksmith.open=false;}
            else if(!title)pause=!pause;
        }
        if(!title&&!pause&&Input.GetKeyDown(KeyCode.M))map=!map;
        Paused=title||pause||map||d.craftOpen||d.inventoryOpen||d.helpOpen||(d.blacksmith&&d.blacksmith.open);
    ...
    [Serializable]class SaveData{public int[] items;public int kills,tier,boss;public bool armor;public int ringA,ringB;public int version,sword,bow,staff,armorItem,weaponMode;public ZoneRecord[] encounters;public int[] seals,objectiveProgress,objectiveAux,weaponUpgrades;public bool[] objectiveStarted,objectiveCompleted,claimed;}
    ...
    void OnGUI()
    {
        ...
        if(map){DrawMap();GUI.matrix=Matrix4x4.identity;return;}if(d.blacksmith&&d.blacksmith.open){DrawForge();GUI.matrix=Matrix4x4.identity;return;}if(d.craftOpen){DrawCraft();GUI.matrix=Matrix4x4.identity;return;}if(d.inventoryOpen){DrawPack();GUI.matrix=Matrix4x4.identity;return;}if(d.helpOpen){DrawHelp();GUI.matrix=Matrix4x4.identity;return;}if(pause){DrawPause();GUI.matrix=Matrix4x4.identity;return;}
    ...
    void DrawForge(){
        var f=d.blacksmith;Panel(new Rect(210,130,860,470));Text(238,151,"EMBER & IRON  /  VILLAGE BLACKSMITH",heading);Text(238,192,"Three permanent forge ranks. Select an owned weapon to preview exact damage and cost.",small,780);
        int row=0;foreach(ItemId id in Enum.GetValues(typeof(ItemId))){if(ItemCatalog.WeaponType(id)<0||!d.Has(id,1))continue;if(Button(new Rect(238,231+row*39,330,34),(f.selected==id?"> ":"")+d.ItemName(id)+"   rank "+f.Level(id)+" / 3"))f.selected=id;row++;}
        ItemId item=f.selected;int type=Mathf.Max(0,ItemCatalog.WeaponType(item)),baseDamage=ItemCatalog.BaseDamage(item,(WeaponKind)type),current=baseDamage+f.Bonus(item);int step=Mathf.Max(2,Mathf.CeilToInt(baseDamage*.1f));
        Text(610,240,d.ItemName(item),heading,400);Text(610,288,"FORGE RANK  "+f.Level(item)+" / 3",small,390);Text(610,324,f.Level(item)>=BlacksmithStation.MaxLevel?"Damage  "+current+"  /  MASTERWORK":"Damage  "+current+"  →  "+(current+step),label,390);Text(610,367,f.Cost(item),small,410);
        GUI.enabled=f.CanUpgrade(item);if(Button(new Rect(610,412,405,47),f.Level(item)>=BlacksmithStation.MaxLevel?"MASTERWORK COMPLETE":"Forge next rank"))f.Upgrade(item);GUI.enabled=true;
        Text(610,477,"Ranks are bounded and stay with this weapon across saves.",small,420);if(Button(new Rect(610,529,180,34),"Close [ESC]"))f.open=false;
    }
    void DrawPack(){
...
public class BetaActor : MonoBehaviour
{
    ...
    void LateUpdate(){
        ...
        if(enemy&&enemy.modifier!=EliteModifier.None&&Time.time>=flashUntil&&enemy.AttackStage==EnemyAttackStage.None)sprite.color=Color.Lerp(Color.white,EliteRules.Color(enemy.modifier),.38f);
